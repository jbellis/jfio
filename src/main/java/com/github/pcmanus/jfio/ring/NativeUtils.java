    ...
    static void loadNativeLibrary(String baseName, String jarLocation) {
        ...
        if (libInJar != null) {
            try {
                ...
                logger.debug("Loaded {} native library from {}", baseName, libfile.getAbsolutePath());
                return;
            } catch (IOException e) {
                logger.warn("Error loading {} native library from jar: {}", baseName, e.getMessage());
            }
        }
        ...
        try {
            ...
            logger.info("Loaded {} native library", baseName);
        } catch (Throwable e) {
